...
impl Kd{
    pub fn new(mut plannets: Vec<Plannet>) -> Kd{
        ...
        if len <= 1{
            ...
            let mean: egui::Vec2 = plannets.iter().map(|p| p.pos.to_vec2()).fold(egui::Vec2::ZERO, |p1, p2| p1 + p2)/len as f32;
            // println!("{:?}: {:?}", len, mean);
            if (min_x - max_x).abs() >= (min_y - max_y).abs(){
                plannets.sort_by(|p1, p2| ord(&p1.pos.x, &p2.pos.x));
                let split = plannets.iter().position(|p| p.pos.x > mean.x).unwrap_or_else(|| {
                    let same: Vec<_> = plannets.iter().enumerate().filter(|(_, p)| p.pos.x == mean.x).collect();
                    same.len()/2+1
                });
                // println!("{:?}: {:?}", split, mean);
                ...
                let split = plannets.iter().position(|p| p.pos.y > mean.y).unwrap_or_else(|| {
                    let same: Vec<_> = plannets.iter().enumerate().filter(|(_, p)| p.pos.y == mean.y).collect();
                    same.len()/2+1
                });
                // println!("{:?}: {:?}", split, mean);
                let other = plannets.split_off(split);
